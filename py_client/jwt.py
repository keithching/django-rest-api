...
class JWTClient:
    ...
    def perform_auth(self):
        ...
        self.write_creds(r.json()) # write the response to a file called creds.json
    ...
    def perform_refresh(self):
        ...
        headers = self.get_headers() # takes in our original access token
        data = {
            "refresh": f"{self.refresh}" 
        }
        endpoint = f"{self.base_endpoint}/token/refresh/"
        r = requests.post(endpoint, json=data, headers=headers) # passes in the refresh token
        if r.status_code != 200:
            self.clear_tokens()
            return False
        refresh_data = r.json()
        if not 'access' in refresh_data:
            ...
            return False
        stored_data = {
            'access': refresh_data.get('access'), # gets us new data from our refresh token
    ...
    def list(self, endpoint=None, limit=3):
        ...
        if endpoint is None or self.base_endpoint not in str(endpoint):
            endpoint = f"{self.base_endpoint}/event_providers/?limit={limit}" 
...
if __name__ == "__main__":
    ...
    # request to our /api/event_providers/ endpoint
